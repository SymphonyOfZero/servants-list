...
// funcion para filtrar v2.0
const filterServant = ()=> {

    // obtienes los valores de los filtros
    let servantClass = document.getElementById('filterByClass').value
    let servantName = document.getElementById('filterByName').value
    let btnEx = document.getElementById("btnEx").dataset.value == "true" ? 'EX' : ""
    let btnA = document.getElementById("btnA").dataset.value == "true" ? 'A' : ""
    let btnB = document.getElementById("btnB").dataset.value == "true" ? 'B' : ""
    let btnC = document.getElementById("btnC").dataset.value == "true" ? 'C' : ""
    let btnD = document.getElementById("btnD").dataset.value == "true" ? 'D' : ""

    let filterOptions = [btnEx,btnA,btnB,btnC,btnD]

    // Filtra por clases
    let newServants = servants.filter((a) => a.class.includes(servantClass))

    // Filtra por nombre
    newServants = newServants.filter((a) => a.name.toLowerCase().includes(servantName.toLowerCase()))

    // Filtra por prioridad
    newServants = newServants.filter((a) => filterOptions.includes(a.priority))

    // Llama a la funcion que imprime los servants resultantes de los filtros
    printServants(newServants)
}
...
const printServants = (filterServant) => {
    let box = document.getElementById('box')

    box.innerHTML = ""

    
    let totalServant = servants.filter(a => ['EX','A'].includes(a.priority))

    let prioEx = filterServant.filter(a => a.priority == 'EX').length
    let prioA = filterServant.filter(a => a.priority == 'A').length
    let prioB = filterServant.filter(a => a.priority == 'B').length
            ...
            `<div class="col-12 col-sm-4 col-md-3 col-lg-2 mt-3 d-flex">
                <div class="card card-servants ">
                    <div class="card-body p-2">
                        ...
                        <span class="text-${priorityColor}"><b>[ Prioridad ${x.priority} ]</b></span>
